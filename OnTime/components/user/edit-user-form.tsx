...
const EditUserForm: React.FC<UserFormProps> = ({ user, styles, showCloseButton, onClose }) => {  
  ...
  const [password, setPassword] = useState(user?.lastName || '');
  ...
  const handleSubmit = async () => {
    ...
    Alert.alert(
      'Submit Updated User',
      `Do you wish to update the user ${firstName} ${lastName} with email as ${email}, password as ${password} and role(s) of ${getSelectedRoles().map(role => role.name).join(", ")}?`,
      [
        {
          text: 'Cancel',
